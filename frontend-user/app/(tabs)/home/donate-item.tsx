...
const CONDITION_OPTIONS: DonationFormData['condition'][] = ['New', 'Good', 'Fair', 'Poor'];
...
export default function DonateItemScreen() {
    ...
    const validateForm = () => {
        ...
        const finalCategoryId = showCustomCategory ? customCategory.trim() : formData.categoryId;

        
        if (!finalCategoryId) {
            newErrors.categoryId = "Item category is required";
        }
    ...
    const uploadImage = async (uri: string) => {
        ...
        try {
            ...
            const type = (extension === 'jpg' || extension === 'jpeg' || extension === 'png' || extension === 'gif' || extension === 'webp') ? `image/${extension}` : "image/jpeg";
    ...
    const handleSubmit = async () => {
        ...
        try {
            const currentItemName = formData.title.trim();
            const currentDescription = formData.description.trim() || undefined;
            const currentCondition = formData.condition;
            const currentImage = formData.photos.length > 0 ? formData.photos[0] : undefined;
            const finalCategoryId = showCustomCategory ? customCategory.trim() : formData.categoryId;

            let endpoint: string;
            let payload: any;

            if (formData.donationType === "giveaway") {
                endpoint = `${API_BASE_URL}/api/mobile/donations`;
                
                const categoryName = showCustomCategory ? finalCategoryId : getCategoryName(true); 
                
                payload = {
                    itemName: `[${categoryName}] ${currentItemName}`, 
                    description: currentDescription,
                    condition: currentCondition,

                    photos: currentImage ? [{ url: currentImage }] : [], 
                };
            } else {
                endpoint = `${API_BASE_URL}/api/mobile/add-item-request`;

                const durationDays = parseInt(formData.duration) || 1;
                const availableFrom = new Date().toISOString();
                const availableUntil = new Date(Date.now() + durationDays * 24 * 60 * 60 * 1000).toISOString();

                if (showCustomCategory) {
                     Alert.alert("Warning", "Duration-based donation requires selecting an existing category ID.");
                     setLoading(false);
                     return;
                }
            ...
            if (response.data.success) {
                Alert.alert(
                    "Thank You!",
                    response.data.message || "Your donation request has been submitted successfully. We'll contact you soon.",
                    [
                        {
                            text: "OK",
                            onPress: () => router.back(),
    ...
    const getCategoryName = (returnIfCustom: boolean = false) => {
        if (showCustomCategory) return returnIfCustom ? customCategory : "Enter custom category";

        const category = allCategories.find((c) => c._id === formData.categoryId);
        ...
        >
            ...
            >
                ...
                <View style={dynamicStyles.formContainer}>
                    ...
                    <View style={dynamicStyles.inputGroup}>
                        <Text style={dynamicStyles.label}>
                            Item Category <Text style={dynamicStyles.required}>*</Text>
                        </Text>

                        {showCustomCategory ? (
                            <View>
                                <TextInput
                                    style={[dynamicStyles.textInput, errors.categoryId && dynamicStyles.inputError]}
                                    placeholder="Enter custom category"
                                    placeholderTextColor={colors.textSecondary}
                                    value={customCategory}
                                    onChangeText={(value) => setCustomCategory(value)}
                                    maxLength={50}
                                    onBlur={() => handleCategoryChange(customCategory)}
                                />
                                <TouchableOpacity
                            ...
                            >
                                <Text
                                    style={
                                        formData.categoryId ? dynamicStyles.selectedText : dynamicStyles.placeholderText
                                    }
                                >
                                    {getCategoryName()}
                                </Text>
                        ...
                        {errors.categoryId && (
                            <Text style={dynamicStyles.errorText}>{errors.categoryId}</Text>
                        )}
                    </View>
                    ...
                    <View style={dynamicStyles.inputGroup}>
                        <Text style={dynamicStyles.label}>
                            Item Condition <Text style={dynamicStyles.required}>*</Text>
                        </Text>
                        <View style={dynamicStyles.donationTypeContainer}>
                            {CONDITION_OPTIONS.map(condition => (
                                <TouchableOpacity
                                    key={condition}
                                    style={[
                                        dynamicStyles.conditionButton,
                                        formData.condition === condition && dynamicStyles.conditionButtonActive,
                                        errors.condition && dynamicStyles.inputError // Apply error style globally if an error exists
                                    ]}
                                ...
                                >
                                    <Text
                                        style={[
                                            ...
                                            formData.condition === condition && dynamicStyles.conditionTextActive,
                                        ]}
                                    >
                                        {condition}
                                    </Text>
                        ...
                        {errors.condition && (
                            <Text style={dynamicStyles.errorText}>{errors.condition}</Text>
                        )}
                        <Text style={dynamicStyles.helperText}>
                            Indicate the current state of the item.
                        </Text>
                    </View>
                    ...
                    {formData.donationType === "duration" && (
                        <View style={dynamicStyles.inputGroup}>
                            ...
                            {errors.duration ? (
                                ...
                                <Text style={dynamicStyles.helperText}>
                                    How many days would you like to lend this item (Max 365)?
                                </Text>
                            )}
                        </View>
                    )}
                    ...
                    {/* Photo Upload */}
                    <View style={dynamicStyles.inputGroup}>
                        <Text style={dynamicStyles.label}>Photos (Optional)</Text>
                        ...
                        <View style={dynamicStyles.pickerContainer}>
                            ...
                            <ScrollView>
                                ...
                                >
                                    ...
                                    <Text
                                        style={[dynamicStyles.pickerItemText, dynamicStyles.otherOptionText]}
                                    >
